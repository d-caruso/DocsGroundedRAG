...
To enable invoice creation on a Payment Link, set [invoice_creation[enabled]](https://docs.stripe.com/api/payment_links/payment_links/create.md#create_payment_link-invoice_creation) to `true`:
...
You can use the [invoice_data](https://docs.stripe.com/api/payment_links/payment_links/create.md#create_payment_link-invoice_creation-invoice_data) hash inside `invoice_creation` to customize the invoice:
...
- If the Checkout Session [creates a new customer](https://docs.stripe.com/api/checkout/sessions/create.md#create_checkout_session-customer) (for example, through the `customer_creation` parameter or `subscription` mode), the browser locale of the user opening the Payment Link URL determines the language.
- If the Checkout Session doesn’t create a new customer, the [language setting](https://dashboard.stripe.com/settings/emails) from your Stripe Dashboard determines the language.
